...
//        settings.setTextZoom(100);//设置WebView中加载页面字体变焦百分比，默认100。//嵌入一个H5页面，使H5页面的字体不会随用户自己调整的系统字体变化而变化
    ...
    with(settings) {
        ...
        useWideViewPort = true // 这个很关键，设置webview推荐使用的窗口
        loadWithOverviewMode = true // 设置webview加载的页面的模式，也设置为true
        displayZoomControls = false // 隐藏webview缩放按钮
        layoutAlgorithm = WebSettings.LayoutAlgorithm.NORMAL //解决字符串换行问题
        mediaPlaybackRequiresUserGesture = false
        builtInZoomControls = false //关闭缩放功能support zoom
